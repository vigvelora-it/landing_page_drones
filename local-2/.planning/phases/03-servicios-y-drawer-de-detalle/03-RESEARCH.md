...
1. **Exactly where does `<main inert>` (or an equivalent wrapper) get applied, given `app/page.tsx` is a Server Component?**
   ...
   - Recommendation: planner should pick one of the four options sketched in Pattern 1 explicitly (leaning toward simplest: skip the extra wiring, rely on native modality, and if a verification step specifically checks for a literal `inert` attribute in rendered HTML, add the smallest version — a wrapping `<div>` around `CapabilitiesSection`'s own sibling sections, driven by the same `useOverlayCoordination` store this phase already introduces).
...
2. **Does the drawer need to handle the case where a user opens a *different* service while one is already open (row A open, user clicks row B)?**
   ...
   - What's unclear: whether that's actually the desired UX (instant content swap while panel stays open) or whether UI-SPEC's mutual-exclusivity design (which only addresses drawer-vs-menu, not drawer-vs-drawer) implicitly assumes a user must close the drawer before opening a different service. UI-SPEC does not address this case explicitly since the row-trigger table doesn't disable *other* rows while one's own drawer is open (only the "selected" indicator changes) — meaning clicking a different row while a drawer is open IS an expected/reachable interaction per the current design, and instant in-place content swap without a close/reopen animation cycle is a reasonable, low-risk default. Flag for the planner as a one-line UX confirmation, not a structural gap — the code above already handles it correctly (no crash, no stuck state), the only question is animation polish (swap instantly vs. briefly close-then-reopen), and the safer/simpler default (instant swap) should ship first with re-open-cycle polish only added if a reviewer explicitly asks for it.
